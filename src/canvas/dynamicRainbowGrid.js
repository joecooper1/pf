export default function dynamicRainbowGrid(ctx, width, height) {
  ...
  for (let i = 0; i < columns; i++) {
    ...
    for (let j = 0; j < rows; j++) {
      arrayOfPoints[i].push({ x: i * 30 - 15, y: j * 30 - 15 });
    }
  }
    ...
    for (let j = 0; j < rows; j++) {
      ...
      if (j > 0) {
        ...
        if (i > 0) {
          ...
          greenValue = (greenValue + arrayOfPoints[i - 1][j].g) / 2 + randomNum();
          blueValue = (blueValue + arrayOfPoints[i - 1][j].b) / 2 + randomNum();
        }
      } else if (i > 0) {
